...
- **The tab title was still `Create Next App`**, and the description still "Generated by
  create next app". Both were scaffold leftovers that had survived every session; the
  metadata in `app/layout.tsx` now names the app and says what it does.
- **The repo, the directory, and the package name stay `demo_lib`.** Only the display name
  changed, so `package.json`, `package-lock.json`, and the checkout path are untouched.

## UI trimmed to what the SE acts on

Six changes, mostly removals. The result area had accumulated controls that looked
useful and were not.

- **Brand colour swatches removed entirely** — no hex codes, no editing. One line now
  states the outcome: colours found on the site and applied, or none found and a neutral
  fallback in use. Extraction has been good enough for a while that the editing controls
  were noise sitting above the thing the SE actually reads.
- **`tagHref` removed from the schema, the template, the sample, and the prompt**, not
  just from the UI. Cards render as plain text; tag page links are added by hand in
  OvalEdge. A field nothing produces and nothing renders is worse than no field, so it is
  gone rather than left unused. `NAV_HREF` stays — the Quick Access links still need it,
  and the validator still accepts a hand-added `#nav/...?query` route in pasted HTML.
- **The logo section lost its instruction list**, keeping the preview, the source/type/size
  line, and the Download button. It duplicated the Next steps block under the preview.
  Next steps absorbed what it said, including the no-logo case — step 2 now reads "source
  the company logo yourself — none was found on the site" when extraction came up empty.
- **The observation during generation carries the waiting panel**: `text-lg`, medium
  weight, on a wider container, with the header stepping back to a quiet label. It is the
  most engaging thing on screen for a minute.
- **"Generation takes about 60 seconds" sits under the Generate button**, so the wait is
  known before the click rather than discovered after it.
- **`cursor-pointer` on every interactive element** — Generate, Copy HTML, Download,
  Publish — plus `disabled:cursor-not-allowed`. Tailwind v4's reset gives buttons
  `cursor: default`, so this needs to be explicit on each one rather than inherited.

## Hero metric as a derived metric

The hero metric is now a *derived* metric whose components are themselves terms in the
same glossary — `Fuel Margin = Fuel Revenue - Fuel Cost of Goods Sold`, with both inputs
defined as their own terms. That is what turns the glossary from a list into a story the
demo can walk: here is a number, here is what it is built from.

Completes the first roadmap item.

- **The model decides which term is the hero metric; the SE no longer picks it.** The
  radio column is gone. Choosing it by hand was backwards — the hero metric has to be
  designed into the glossary, not selected from it afterwards, because its components must
  also be terms.
- **The cross-reference is validated in Zod, not hoped for.** `componentTerms` holds 2–3
  names, each of which must match the `termName` of another term in the same glossary, all
  distinct, never the hero itself, only on the hero, and every one of them named in the
  formula. A component pointing at a term nobody defined is invalid, and the error says
  which name is missing.
- **The prompt sequences the design rather than describing the output.** It gives a worked
  example and five ordered steps — choose a derived metric, decide its inputs, write those
  inputs as full terms, then write the metric referencing them — plus an explicit "do not
  pick a term first and invent components for it afterwards". A live run on a fuel retailer
  produced Fuel Margin with both components defined, valid on the first attempt.
- **Renaming a component is caught before publishing.** Term names stay editable, so the
  UI re-checks the cross-reference client-side and warns when an edit has broken it, rather
  than letting the server reject the publish after the click.
- **After publishing, the UI lists the exact links to create by hand** —
  `Fuel Margin (1234) → Fuel Revenue (1235)` — with the created termIds beside each name.
  OvalEdge's term-relationship endpoint is authenticated by browser session rather than by
  our token, so this cannot be automated; the ids are there because finding a term by name
  in the OvalEdge UI is the slow part. See [KNOWN-ISSUES.md](KNOWN-ISSUES.md).
- The sample fixture was rebuilt around Freight Margin = Freight Revenue - Freight Cost, so
  `check:schema` exercises a glossary that actually satisfies the cross-reference.
